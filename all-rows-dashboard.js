...
async function scrapeData() {
  ...
  try {
    ...
    await page.goto(
      "https://zm.bitmoji-zmlh.com/skinmgr/#/skinmgr/recordsList",
      {
        waitUntil: "networkidle2",
        timeout: 60000,
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        try {
          ...
          if (cells.length >= 11) {
            ...
            const column0 = await cells[0].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const id = await cells[1].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            ...
            const customerInfo = await cells[3].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const gender = await cells[4].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const deviceNumber = await cells[5].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const account = await cells[6].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const testTime = await cells[7].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const testStatus = await cells[8].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const remarks = await cells[9].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            const operation = await cells[10].evaluate(
              (el) => el.textContent?.trim() || ""
            );
            ...
            try {
              ...
              if (linkElement) {
                viewProfileLink = await linkElement.evaluate(
                  (el) => el.href || ""
                );
              }
            } catch (e) {
            ...
            if (id && id !== "ID" && !isNaN(parseInt(id))) {
              ...
              console.log(
                `✅ Extracted item ${i}: Row ${i}, Column0: "${column0}", ID: ${id}, Time: ${testTime}`
              );
            } else {
              console.log(
                `⚠️ Row ${i} appears to be header or invalid data: ID="${id}"`
              );
            }
          } else {
...
app.listen(PORT, () => {
  ...
  console.log(
    "🔍 This dashboard will extract ALL rows including the first one"
  );
