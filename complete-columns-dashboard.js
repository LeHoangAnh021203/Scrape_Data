...
// Scraping function with all columns
async function scrapeData() {
  ...
  try {
    ...
    if (dataTable && maxRows > 1) {
      ...
      for (let i = 1; i < rows.length; i++) {
        try {
          ...
          if (cells.length >= 11) {
            ...
            if (id) {
              extractedData.push({
                column0, // First column that was missing
                id,
                picture,
                customerInfo,
                gender,
              ...
              console.log(`✅ Extracted item ${i}: Column0: "${column0}", ID: ${id}, Time: ${testTime}`);
            } else {
              console.log(`❌ Row ${i} has no ID`);
            }
          } else {
      ...
      console.log(`📊 Extracted ${extractedData.length} items with ALL columns`);
...
app.listen(PORT, () => {
  console.log(`🚀 Complete Columns Dashboard running at http://localhost:${PORT}`);
  console.log('📊 Open your browser and navigate to http://localhost:3000');
  console.log('🔍 This dashboard will extract ALL columns from the table');
